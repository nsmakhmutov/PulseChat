...
# ── Принудительный мут участника хостом ──────────────────────────────────────
#
# Хост (host_order[0]) может заглушить любого участника: выключить ему микрофон
# и динамики. Снять мут хост НЕ МОЖЕТ — это намеренное ограничение.
# Участник видит тост «Вас заглушил администратор» и кнопки mic/deaf блокируются.
#
# Поток сигналов:
#   хост → CMD_HOST_MUTE (target_uid) → сервер
#   сервер → CMD_FORCE_MUTED → цель (только ей)
#
CMD_HOST_MUTE   = 'host_mute'    # хост → сервер: заглушить target_uid
CMD_FORCE_MUTED = 'force_muted'  # сервер → цель: принудительный мут от хоста
