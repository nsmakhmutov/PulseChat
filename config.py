...
# ── Постоянный чат (Chat) ─────────────────────────────────────────────────────
# История живёт в памяти каждого клиента (до CHAT_HISTORY_MAX сообщений).
# Сервер только транслирует — ничего не хранит.
# При подключении нового участника хост присылает последние N сообщений.
#
# Поток chat_msg:
#   клиент → CMD_CHAT_MSG → сервер → все в комнате (с nick/uid/avatar/ts)
#
# Поток истории при входе:
#   новый клиент → 'chat_history_req' → сервер → хост (host_order[0])
#   хост         → CMD_CHAT_HISTORY (target_uid=новый) → сервер → новый клиент
#
CMD_CHAT_MSG         = 'chat_msg'          # клиент → сервер → все в комнате
CMD_CHAT_HISTORY     = 'chat_history'      # хост → сервер → новый участник
CMD_CHAT_HISTORY_REQ = 'chat_history_req'  # новый участник → сервер → хост
CMD_CHAT_MEDIA       = 'chat_media'        # клиент → сервер → все: вложение

CHAT_MSG_MAX_LEN  = 500    # символов — максимальная длина одного сообщения
CHAT_HISTORY_MAX  = 500    # сообщений в памяти на каждом клиенте
# Максимальный размер вложения (base64 строка).
# 10 000 000 символов base64 ≈ 7.5 MB исходного файла (достаточно для коротких видео).
CHAT_MEDIA_MAX_B64 = 10_000_000

# ── Принудительный мут микрофона участника хостом ────────────────────────────
#
# Хост (host_order[0]) может выключить микрофон любого участника.
# Уши (deaf) НЕ трогаются. Участник может включить mic обратно сам в любой момент.
# Снять мут через эту команду нельзя — намеренное ограничение.
#
# Поток: хост → CMD_HOST_MUTE (target_uid) → сервер → CMD_FORCE_MUTED → цель
#
CMD_HOST_MUTE   = 'host_mute'    # хост → сервер: выключить mic target_uid
CMD_FORCE_MUTED = 'force_muted'  # сервер → цель: mic выключен хостом
